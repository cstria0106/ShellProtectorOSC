...
void DrawUI(OSC &osc) {
  ...
  ImGui::InputText("##Password", password, key_length + 1,
  ...
  if (parameter_multiplexing) {
    ...
    DisplayTooltip(
        "The wait time before sending the next OSC data. If you can't decrypt "
        "when viewed by other users, try increasing this value.");
    ImGui::SameLine();
    ImGui::SetNextItemWidth(50);
...
void DrawLog(OSC &osc) {
  ...
  ImGui::Separator();
  ImGui::BeginChild("Scrolling", ImVec2(0.f, 0.f), true,
